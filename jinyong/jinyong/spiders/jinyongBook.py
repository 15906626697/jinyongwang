...
class JinyongbookSpider(scrapy.Spider):
    name = 'jinyongBook'
    ...
    def parse_list(self, response):
        ...
        for book in book_list:
            ...
            item["book_name"] = response.xpath(
                '//*[@id="pu_box"]/div[@class="main"]/div[@class="booklist"]/h1[@class="title"]/span/text()').extract_first()
            item["book_status"] = response.xpath(
                '//*[@id="pu_box"]/div[@class="main"]/div[@class="booklist"]/h1[@class="title"]/text()').extract_first()
            item["author"] = response.xpath(
                '//*[@id="pu_box"]/div[@class="main"]/div[@class="booklist"]/p[@class="author"]/a/text()').extract_first()
            item["publish_time"] = response.xpath(
                '//*[@id="pu_box"]/div[@class="main"]/div[@class="booklist"]/p[2]/text()').extract_first()
            item["Publishing_house"] = response.xpath(
                '//*[@id="pu_box"]/div[@class="main"]/div[@class="booklist"]/p[3]/text()').extract_first()
            # print(item)
            request = scrapy.Request(url=item["title_url"], callback=self.parse_detail, dont_filter=True, meta={'item': deepcopy(item)})
